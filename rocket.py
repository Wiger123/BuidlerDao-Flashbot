...
# Sniper 类
class Sniper(QObject):
    # PyQt 信号
    logSignal = pyqtSignal(str)
    priceSignal = pyqtSignal(str)
    buyPriceSignal = pyqtSignal(str)
    lossPriceSignal = pyqtSignal(str)
    profitPriceSignal = pyqtSignal(str)
    ...
    def __init__(self, parent=None):
        ...
        self.address_1 = ''
        # BNB/USDT
        self.token_1_check_sum_address = ''
        # 交易结束地址: 新币地址, 需要用户输入
        self.address_2 = ''
        # 新币
        self.token_2_check_sum_address = ''
        # 地址列表
        self.addresses_buy = []
        self.addresses_sell = []
    ...
    def approve_usdt(self):
        ...
        # 打印结果
        self.logSignal.emit("授权结果: {0}".format(th))
        print("授权结果: {0}".format(th))
    ...
    def buy_market_bnb(self):
        ...
        if token_balance > 0:
            self.logSignal.emit("交易成功")
            print('交易成功')
            # 设置购买状态
            self.buy_flag = True
        else:
            self.logSignal.emit("交易失败")
            print('交易失败')

        # 打印结果
        self.logSignal.emit("市价买入: {0}".format(th))
        print("市价买入: {0}".format(th))
        
        # 获取数量
        logs_1 = th['logs'][0]['data']
        logs_2 = th['logs'][1]['data']
        logs_3 = th['logs'][2]['data']
        # print("16进制数据: In: {0}; Out: {1};".format(logs_1, logs_3))

        # 16 进制转 10 进制
        logs_1 = int(logs_1, 16)
        logs_3 = int(logs_3, 16)
        # print("10进制数据: In: {0}; Out: {1};".format(logs_1, logs_3))
        ...
        print("实际数据: In: {0}; Out: {1};".format(self.act_count_in, self.act_count_out))

        # 交易单价
        self.act_price = self.act_count_in / self.act_count_out * self.real_time_bnb_price
        print("买入单价: {0} USDT".format(self.act_price))
        self.buyPriceSignal.emit('%.20f' % self.act_price)
    ...
    def sell_market(self, count_in_s):
        ...
        # 打印结果
        self.logSignal.emit("市价卖出: {0}".format(th))
        print("市价卖出: {0}".format(th))
    ...
    def query_balance(self):
        ...
        # 打印余额
        self.logSignal.emit("BNB 余额: {0}; {1} 余额: {2}".format(bnb_balance, token_name, token_balance))
        print("BNB 余额: {0}; {1} 余额: {2}".format(bnb_balance, token_name, token_balance))
    ...
    def find_pair_addr(self):
        ...
        # 交易对地址
        self.logSignal.emit("合约地址: {0}; 交易对地址: {1}".format(self.token_2_check_sum_address, pair_address))
        # print("合约地址: {0}; 交易对地址: {1}".format(self.token_2_check_sum_address, pair_address))
    ...
    def getReserveOfToken(self):
        ...
        while self.liquid_flag == False:
            ...
            # 打印池子数据
            self.logSignal.emit("池子大小: BNB 数: {0}, Token 数: {1}".format(reserve1, reserve2))
            # print("池子大小: BNB/USDT 数: {0}, Token 数: {1}".format(reserve1, reserve2))
            ...
            if reserve1 >= self.token_1_min:
                ...
                # 开始抢币
                self.logSignal.emit("BNB 数量满足, 开始抢币!")
                # print("BNB/USDT 数量满足, 开始抢币!")
            ...
            # 检测 代币 数量
                ...
                # 开始抢币
                self.logSignal.emit("Token 数量满足, 开始抢币!")
                # print("Token 数量满足, 开始抢币!")
    ...
    def buy_market_thread(self):
        ...
        while self.liquid_flag == False:
            ...
            if count_liqul % self.print_interval == 0:
                self.logSignal.emit("已搜寻 {0} 次, 仍未开盘".format(count_liqul))
                print("已搜寻 {0} 次, 仍未开盘".format(count_liqul))
        ...
        while self.buy_flag == False:
            ...
            try:
                ...
                self.logSignal.emit("地址错误 或 BNB/USDT 余额不足")
                print("地址错误 或 BNB/USDT 余额不足")
    ...
    def sell_model_type(self):
        ...
        if token_balance_bag <= 0:
            self.logSignal.emit("背包中代币数量为 0, 不进行出售")
            print("背包中 Token 数量为 0, 不进行出售")
        else:
            # 出售数目
            ...
            try:
                ...
                self.logSignal.emit("手续费不足 或 余额不足")
                print("手续费不足 或 Token 余额不足")
    ...
    def get_price_real_time_bnb(self):
        ...
        while True:
            ...
            # 价格显示
            self.priceSignal.emit('%.20f' % self.real_time_price)
            # print('当前 Token 价格为: %.20f USDT' % self.real_time_price)
    ...
    def get_token_decimal(self):
        ...
        # 显示精度
        print("Token 精度为: {0};".format(self.decimal_2))
        # 返回
        return
    ...
    def prof_sell(self):
        ...
        bnb_balance_bag, token_balance_bag = self.query_balance()
        if token_balance_bag <= 0:
            self.logSignal.emit("背包中代币数量为 0, 不进行止盈")
            print("背包中 Token 数量为 0, 不进行止盈")
            return

        # 设置止盈价格
        self.control_prof_price = self.act_price * Decimal(1 + self.control_prof_per / 100)
        print('止盈触发价格: {0} USDT'.format(self.control_prof_price))
        self.profitPriceSignal.emit('%.20f' % self.control_prof_price)
    ...
    def loss_sell(self):
        ...
        bnb_balance_bag, token_balance_bag = self.query_balance()
        if token_balance_bag <= 0:
            self.logSignal.emit("背包中代币数量为 0, 不进行止损")
            print("背包中 Token 数量为 0, 不进行止损")
            return

        # 设置止损价格
        self.control_loss_price = self.act_price * Decimal(1 - self.control_loss_per / 100)
        print('止损触发价格: {0} USDT'.format(self.control_loss_price))
        self.lossPriceSignal.emit('%.20f' % self.control_loss_price)
    ...
    def print_all_para(self):
        print("BNB/USDT: {0};".format(self.money_type))
        print("合约地址: {0}; 新币精度: {1};".format(self.token_2_check_sum_address, self.decimal_2))
        print("钱包地址: {0}; 钱包私钥: {1};".format(self.my_check_sum_address, self.my_private_key))
        print("截止时间: {0}; 买入滑点: {1}; Buy Gas Price: {2}; Buy Gas Limit: {3}; Sell Gas Price: {4}; Sell Gas Limit: {5}; BNB/USDT_MIN: {6}; TOKEN_MIN: {7};".format(self.deadline, self.slippage_buy, self.buy_gas_price, self.buy_gas_limit, self.sell_gas_price, self.sell_gas_limit, self.token_1_min, self.token_2_min))
        print("买入花费金额: {0};".format(self.count_token_1))
        print("买入单价: {0}; 消耗 BNB/USDT: {1}; 获取 Token: {2};".format(self.act_price, self.act_count_in, self.act_count_out))
        print("卖出百分比: {0};".format(self.per_token_2))
        print("止损百分比: {0}; 止损价格: {1};".format(self.control_loss_per, self.control_loss_price))
        print("止盈百分比: {0}; 止盈价格: {1};".format(self.control_prof_per, self.control_prof_price))
    ...
    def set_buy_token_address(self, address):
        ...
        try:
            ...
            self.logSignal.emit("买入token地址不合法")
            return False
        return True
    ...
    def set_para(self, address, private_key, time_period, slippage_buy, gas_price_buy, gas_limit_buy, gas_price_sell, gas_limit_sell, token_1_min, token_2_min):
        ...
        try:
            ...
            self.logSignal.emit("钱包地址不合法")
            return False
        self.logSignal.emit("信息保存成功")
        return True
